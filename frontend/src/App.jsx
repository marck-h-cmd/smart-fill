...
import { Database, Sun, Moon, Activity, Server } from 'lucide-react';
...
function App() {
  ...
  const loadDashboardData = useCallback(async () => {
    setLoading(true);
    try {
      const activeRes = await axios.get('http://localhost:5000/api/databases/active');
      const db = activeRes.data.data;
      setActiveDb(db);

      if (db && db.id) {
        const statsRes = await axios.post(`http://localhost:5000/api/databases/${db.id}/stats`);
        setDbStats(statsRes.data.data);

        const fragRes = await axios.post(`http://localhost:5000/api/databases/${db.id}/fragmentation`);
  ...
  useEffect(() => {
    if (activeView === 'monitoring') loadDashboardData();
      ...
      <header className="h-14 hairline-b flex items-center justify-between px-6 bg-surface z-10 transition-colors duration-300">
        ...
        <div className="flex items-center h-full">
          <nav className="flex gap-1 h-full mr-6">
            ...
            <NavTab label="MONITOREO" view="monitoring" current={activeView} set={setActiveView} />
            <NavTab label="BD CONFIG" view="db_config" current={activeView} set={setActiveView} />
      ...
      <main className="flex-1 flex overflow-hidden">
        <div className="w-80 hairline-r bg-panel/50 flex flex-col transition-colors duration-300">
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            <div className="font-mono text-xs text-fgMuted space-y-2">
              ...
              {activeDb && <p className="text-accent">{'>'} BD activa: {activeDb.name}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-surface p-10 relative transition-colors duration-300">
          <div className="max-w-4xl mx-auto">
            ...
            {activeView === 'db_config' && <DatabaseConfig />}
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-3 gap-px bg-border border border-border">
        <KPICard title="TABLAS CRÍTICAS" value="--" sub="Requieren reconstrucción" />
        <KPICard title="FILLFACTOR PROM." value="--" sub="A nivel de sistema" />
        <KPICard title="ÚLTIMO ESCANEO" value="--" sub="Sin datos" />
      </div>
      ...
      <div className="panel p-8 mt-8 flex flex-col items-center justify-center min-h-[300px]">
        ...
        <p className="text-fgMuted font-mono text-sm">
          Configura una base de datos activa en "BD CONFIG" y ve a "MONITOREO" para ver datos en tiempo real.
        </p>
      </div>
    </div>
  );
...
function MonitoringView({ dbStats, fragData, activeDb, loading, onRefresh }) {
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-light tracking-tight mb-2">Monitoreo en Tiempo Real</h2>
          <p className="text-fgMuted">
            {activeDb
              ? `Base de datos: ${activeDb.name} (${activeDb.host}/${activeDb.database})`
              : 'Sin base de datos activa. Configura una en "BD CONFIG".'}
          </p>
        </div>
        <button onClick={onRefresh} className="btn-accent py-2 px-4 text-sm font-mono flex items-center gap-2">
          <Activity size={16} /> Actualizar
        </button>
      </div>

      {loading && <p className="text-fgMuted font-mono text-sm">Cargando datos...</p>}
